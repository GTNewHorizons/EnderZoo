...
public class SpawnConfigParser extends DefaultHandler {
    ...
    public void startElement(String uri, String localName, String qName, Attributes attributes) throws SAXException {
        ...
        if (ELEMENT_ROOT.equals(localName)) {
            ...
            if ((currentEntry == null || currentFilter == null) && !invalidEntryElement) {
                Log.warn(
                        ELEMENT_BIOME + " found outside an "
                                + ELEMENT_ENTRY
                                + " and/or "
                                + ELEMENT_FILTER
            ...
            if (currentEntry == null && !invalidEntryElement) {
                Log.warn(
                        ELEMENT_DIM_EXCLUDE + " found outside an "
                                + ELEMENT_ENTRY
                                + " and/or "
                                + ELEMENT_FILTER
                                + " element. It will be ignored");
    ...
    private void parseDimExclude(Attributes attributes) {
        ...
        currentEntry.addDimensioFilter(
                new DimensionFilter(
                        getIntValue(ATT_ID_START, attributes, Integer.MAX_VALUE),
                        getIntValue(ATT_ID_END, attributes, Integer.MAX_VALUE)));
    ...
    private void parseEntry(Attributes attributes) {
        ...
        if (creatureType != null) {
            try {
                ...
                Log.warn(
                        "Invalid value specified for " + ATT_CREATURE_TYPE
                                + " in entry "
                                + id
                                + " using default value "
                                + currentEntry.getCreatureType()
                                + " error: "
                                + e);
            }
        }
        ...
        if (minGrp != -1) {
            if (minGrp < 0) {
                Log.warn(
                        "Value less than 0 found for " + ATT_MIN_GRP
                                + " in entry "
                                + id
                                + " using default value "
                                + currentEntry.getMinGroupSize());
            } else {
        ...
        if (maxGrp != -1) {
            if (maxGrp < currentEntry.getMinGroupSize()) {
                Log.warn(
                        "Value for " + ATT_MAX_GRP
                                + " in entry "
                                + id
                                + " less than "
                                + ATT_MIN_GRP
                                + " using default "
                                + currentEntry.getMaxGroupSize());
            } else {
