...
public final class MobSpawns {
    ...
    public void addSpawn(ISpawnEntry entry) {
        ...
        Class<? extends EntityLiving> clz = (Class<? extends EntityLiving>) EntityList.stringToClassMapping
                .get(entry.getMobName());
        if (clz == null) {
        ...
        for (IBiomeFilter filter : entry.getFilters()) {
            ...
            EntityRegistry.addSpawn(
                    clz,
                    entry.getRate(),
                    entry.getMinGroupSize(),
                    entry.getMaxGroupSize(),
                    entry.getCreatureType(),
                    biomes);
        }
        if (PRINT_DETAIL) {
            System.out.println();
