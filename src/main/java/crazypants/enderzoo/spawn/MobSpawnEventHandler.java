...
public class MobSpawnEventHandler {
    ...
    public void onEntityJoinWorld(EntityJoinWorldEvent evt) {
        ...
        } else if (Config.globalDifficultyModifierEnabled && evt.entity instanceof IMob
                && evt.entity instanceof EntityLivingBase) {
                    EntityLivingBase ent = (EntityLivingBase) evt.entity;
                    if (!ent.getEntityData().getBoolean(APPLIED_KEY)) {
    ...
    protected void addjustBaseHealth(EntityLivingBase ent, double healthModifier) {
        ...
        // System.out.println("MobSpawnEventHandler.addjustBaseHealth: Base health changed from: " + curValue + " to " +
        // newValue);
    }
    ...
    protected void adjustBaseAttack(EntityLivingBase ent, double attackModifier) {
        ...
        att.setBaseValue(newValue);
        // System.out.println("MobSpawnEventHandler.adjustBaseAttack: base attack changed from " + curValue + " to " +
        // newValue);
    }

    @SubscribeEvent
    public void onBlockHarvest(HarvestDropsEvent event) {
        if (!(event.harvester instanceof FakePlayer) && Config.direSlimeEnabled
                && !event.isCanceled()
                && (event.block instanceof BlockDirt || event.block instanceof BlockGrass)
                ...
                && !ForgeHooks
                        .isToolEffective(event.harvester.getCurrentEquippedItem(), event.block, event.blockMetadata)
