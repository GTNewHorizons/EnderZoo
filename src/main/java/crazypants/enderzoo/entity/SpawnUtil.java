...
public class SpawnUtil {
    ...
    public static boolean findClearGround(World world, Point3i startingLocation, Point3i clearLocation, int horizRange,
            int vertRange, boolean checkForLivingEntities) {
        ...
        for (int xOff = -horizRange; xOff <= horizRange && !foundTargetSpace; xOff++) {
            ...
            for (int zOff = -horizRange; zOff <= horizRange && !foundTargetSpace; zOff++) {
                ...
                foundTargetSpace = SpawnUtil
                        .seachYForClearGround(clearLocation, world, vertRange, checkForLivingEntities);
                if (!foundTargetSpace) {
    ...
    public static boolean seachYForClearGround(Point3i target, World world, int searchRange,
        ...
        if (foundY) {
            for (int i = 0; i < searchRange && !onGround; i++) {
                onGround = !world.isAirBlock(target.x, target.y - 1, target.z)
                        && !isLiquid(world, target.x, target.y - 1, target.z);
                if (!onGround) {
                    target.y--;
    ...
    public static boolean isSpaceAvailableForSpawn(World worldObj, EntityLiving entity, EntityCreature asCreature,
            boolean checkEntityCollisions, boolean canSpawnInLiquid) {
