...
public class EntityAIMountedAttackOnCollide extends EntityAIBase {
    ...
    public EntityAIMountedAttackOnCollide(EntityCreature attacker, Class<?> targetClass, double speedTowardsTarget,
            double speedTowardsTargetMounted, boolean longMemory) {
        this(attacker, speedTowardsTarget, speedTowardsTargetMounted, longMemory);
        this.classTarget = targetClass;
    }

    public EntityAIMountedAttackOnCollide(EntityCreature attacker, double speedTowardsTarget,
            double speedTowardsTargetMounted, boolean longMemory) {
    ...
    public boolean continueExecuting() {
        ...
        return entitylivingbase == null ? false
                : (!entitylivingbase.isEntityAlive() ? false
                        : (!longMemory ? !getNavigator().noPath()
    ...
    public void updateTask() {
        ...
        if ((longMemory || attacker.getEntitySenses().canSee(target)) && pathUpdateTimer <= 0
                && (targetPosX == 0.0D && targetPosY == 0.0D && targetPosZ == 0.0D
                        || target.getDistanceSq(targetPosX, targetPosY, targetPosZ) >= 1.0D
            ...
            if (getNavigator().getPath() != null) {
                ...
                if (finalPathPoint != null
                        && target.getDistanceSq(finalPathPoint.xCoord, finalPathPoint.yCoord, finalPathPoint.zCoord)
                                < 1) {
