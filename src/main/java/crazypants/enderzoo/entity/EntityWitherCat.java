...
public class EntityWitherCat extends EntityMob implements IOwnable<EntityWitherCat, EntityWitherWitch>, IEnderZooMob {
    ...
    protected void updateAttackDamage(float growthRatio) {
        IAttributeInstance att = EntityUtil
                .removeModifier(this, SharedMonsterAttributes.attackDamage, ATTACK_BOOST_MOD_UID);
        if (growthRatio == 0) {
            return;
        }
    ...
    protected void updateHealth(float growthRatio) {
        IAttributeInstance att = EntityUtil
                .removeModifier(this, SharedMonsterAttributes.maxHealth, HEALTH_BOOST_MOD_UID);
        if (growthRatio == 0) {
            return;
        }
    ...
    private void spawnParticles() {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            EntityFX fx = Minecraft.getMinecraft().renderGlobal
                    .doSpawnParticle("spell", startX + xOffset, startY + yOffset, startZ + zOffset, 0.0D, 0.0D, 0.0D);
            if (fx != null) {
